def recite(start_verse, end_verse):
    ...
    for n in range(start_verse, end_verse + 1):
        days_list = ['first', 'second', 'third', 'fourth', 'fifth', 'sixth', 'seventh', 'eighth', 'ninth', 'tenth',
                     'eleventh', 'twelfth']
        lyrics_list = ['a Partridge in a Pear Tree', 'two Turtle Doves', 'three French Hens', 'four Calling Birds',
                       'five Gold Rings', 'six Geese-a-Laying', 'seven Swans-a-Swimming', 'eight Maids-a-Milking',
                       'nine Ladies Dancing', 'ten Lords-a-Leaping', 'eleven Pipers Piping', 'twelve Drummers Drumming']
        separator = ', '
        lyrics_list_slice = lyrics_list[n - 1: 0: -1]
        if n == 1:
            variable_text = lyrics_list[n - 1]
        else:
            variable_text = separator.join(lyrics_list_slice) + f', and {lyrics_list[0]}'
